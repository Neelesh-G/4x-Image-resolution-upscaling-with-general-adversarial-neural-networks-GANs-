...
def load_data_from_dirs(dirs):
    ...
    for d in dirs:
        for f in os.listdir(d): 
            ...
            #image = mpimg.imread(os.path.join(d,f))
                #f.close()
...
def load_data_from_dirs_resize(dirs, size):
    files = []
    file_names = []
    count = 0
    for d in dirs:
...
def load_data(directory):
    ...
    files = load_data_from_dirs(load_path(directory))
    return files
...
def train(epochs=1, batch_size=128):
    ...
    generator.compile(loss=vgg_loss, optimizer=adam,metrics=['accuracy'])
    ...
    for e in range(1, epochs+1):
        ...
        for _ in range(batch_count):
            ...
            rand_nums = np.random.randint(1, x_train_hr.shape[0], size=batch_size)
            image_batch_hr = x_train_hr[rand_nums]
            image_batch_lr = x_train_lr[rand_nums]
